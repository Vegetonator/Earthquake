require([
  ...
  const map = new Map({
    basemap: new Basemap({
      baseLayers: [
        new TileLayer({
          url: "https://tiles.arcgis.com/tiles/nGt4QxSblgDfeJn9/arcgis/rest/services/Vibrant/MapServer?cacheKey=8a2128d0f1a0297f",
          opacity: 0.7,
          minScale: 0,
        }),
      ],
  ...
  const view = new SceneView({
    ...
    environment: {
      ...
      lighting: {
        date: "Sun Nov 19 2023 21:04:41 GMT+0200 (Central European Summer Time)",
      },
      starsEnabled: false,
  ...
  /*****************************************
   * Create GeoJSONLayer with usgs data
   *****************************************/
  ...
  const earthquakesLayer = new GeoJSONLayer({
    ...
    title: "Earthquakes in the last 7 days",
    popupTemplate: {
  ...
  const secondSymbolLayer = {
    type: "icon",
    resource: { primitive: "circle" },
    material: { color: [255, 0, 255, 0] },
    outline: { color: [255, 0, 255, 0.7], size: 1 },
    size: 20,
  };
  ...
  const thirdSymbolLayer = {
    ...
    resource: { primitive: "circle" },
    material: { color: [255, 0, 255, 0] },
    outline: { color: [255, 0, 255, 0.5], size: 1 },
    size: 40,
  };
